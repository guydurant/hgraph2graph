...
class HierMPNEncoder(nn.Module):
    ...
    def forward(self, tree_tensors, graph_tensors):
        ...
        hroot = self.embed_root(
            hmess, tensors, [st for st, le in tree_tensors[-1]])
...
class IncMPNEncoder(MPNEncoder):
    ...
    def forward(self, tensors, h, num_nodes, subset):
        ...
        node_buf = torch.zeros(
            num_nodes, self.hidden_size, device=fmess.device)
...
class IncHierMPNEncoder(HierMPNEncoder):
    ...
    def __init__(self, vocab, avocab, rnn_type, embed_size, hidden_size, depthT, depthG, dropout):
        super(IncHierMPNEncoder, self).__init__(vocab, avocab, rnn_type,
                                                embed_size, hidden_size, depthT, depthG, dropout)
    ...
    def get_sub_tensor(self, tensors, subset):
        ...
        fnode, fmess = fnode.index_select(
            0, subnode), fmess.index_select(0, submess)
        agraph, bgraph = agraph.index_select(
            0, subnode), bgraph.index_select(0, submess)
    ...
    def forward(self, tree_tensors, inter_tensors, graph_tensors, htree, hinter, hgraph, subtree, subgraph):
        ...
        if len(subgraph[0]) + len(subgraph[1]) > 0:
            sub_graph_tensors = self.get_sub_tensor(graph_tensors, subgraph)[
                :-1]  # graph tensor is already embedded
