...
class IncBase(object):
    ...
    def __init__(self, batch_size, node_fdim, edge_fdim, max_nodes=100, max_edges=200, max_nb=12):
        ...
        self.fnode = torch.zeros(
            max_nodes * batch_size, node_fdim).long().to(device)
    ...
    def add_edge(self, i, j, feature=None):
        ...
        in_edges = [self.edge_dict[(k, i)]
                    for k in self.graph.predecessors(i) if k != j]
...
class IncTree(IncBase):
    ...
    def __init__(self, batch_size, node_fdim, edge_fdim, max_nodes=100, max_edges=200, max_nb=12, max_sub_nodes=30):
        super(IncTree, self).__init__(batch_size, node_fdim,
                                      edge_fdim, max_nodes, max_edges, max_nb)
        self.cgraph = self.fnode.new_zeros(
            max_nodes * batch_size, max_sub_nodes)
...
class IncGraph(IncBase):
    ...
    def __init__(self, avocab, batch_size, node_fdim, edge_fdim, max_nodes=100, max_edges=300, max_nb=10):
        super(IncGraph, self).__init__(batch_size, node_fdim,
                                       edge_fdim, max_nodes, max_edges, max_nb)
        self.avocab = avocab
        self.mol = Chem.RWMol()
    ...
    def add_mol(self, batch_idx, smiles, inter_label, nth_child):
        ...
        for bond in emol.GetBonds():
            ...
            if existing_bond is None:
                ...
                self.add_edge(a1, a2, self.get_mess_feature(
                    bond.GetBeginAtom(), bond_type, nth_child if a2 in attached else 0))
                self.add_edge(a2, a1, self.get_mess_feature(
                    bond.GetEndAtom(), bond_type, nth_child if a1 in attached else 0))
            else:
                attached.extend([(a1, a2), (a2, a1)])
            new_bonds.extend(
                [self.edge_dict[(a1, a2)], self.edge_dict[(a2, a1)]])
    ...
    def get_assm_cands(self, cluster, used, smiles):
        ...
        if emol.GetNumAtoms() == 1:
            ...
            attach_points = [atom.GetIdx() for atom in emol.GetAtoms()
                             if atom.GetAtomMapNum() == 1]
        ...
        if inter_size == 1:
            ...
            anchor_smiles = [get_anchor_smiles(
                emol, a, idxfunc) for a in anchors]
        else:
            # all attach points are labeled with 1
            anchors = [a for a in attach_points if is_anchor(
                emol.GetAtomWithIdx(a), [0])]
        ...
        if inter_size == 1:
            ...
            cands = [cluster2[i: i + inter_size]
                     for i in range(len(cluster))]  # not pairs if inter_size >= 3
