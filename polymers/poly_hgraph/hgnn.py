...
def make_cuda(tensors):
    ...
    tree_tensors = [make_tensor(x).to(device).long()
                    for x in tree_tensors[:-1]] + [tree_tensors[-1]]
    graph_tensors = [make_tensor(x).to(device).long()
...
class HierVAE(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.encoder = HierMPNEncoder(args.vocab, args.atom_vocab, args.rnn_type,
                                      args.embed_size, args.hidden_size, args.depthT, args.depthG, args.dropout)
        self.decoder = HierMPNDecoder(args.vocab, args.atom_vocab, args.rnn_type, args.embed_size,
    ...
    def sample(self, batch_size):
        ...
        #tree_vecs = torch.randn(batch_size, self.latent_size).to(device)
        #graph_vecs = torch.randn(batch_size, self.latent_size).to(device)
...
class HierVGNN(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.encoder = HierMPNEncoder(args.vocab, args.atom_vocab, args.rnn_type,
                                      args.embed_size, args.hidden_size, args.depthT, args.depthG, args.dropout)
        self.decoder = HierMPNDecoder(args.vocab, args.atom_vocab, args.rnn_type, args.embed_size,
                                      args.hidden_size, args.hidden_size, args.diterT, args.diterG, args.dropout, attention=True)
        self.encoder.tie_embedding(self.decoder.hmpn)
    ...
    def translate(self, tensors, num_decode, enum_root, greedy=True):
        ...
        z_tree = torch.randn(
            batch_size, 1, self.latent_size).expand(-1, tree_vecs.size(1), -1).to(device)
        z_graph = torch.randn(
            batch_size, 1, self.latent_size).expand(-1, graph_vecs.size(1), -1).to(device)
    ...
    def forward(self, x_graphs, x_tensors, y_graphs, y_tensors, y_orders, beta):
        ...
        diff_tree_vecs, tree_kl = self.rsample(
            diff_tree_vecs, self.T_mean, self.T_var)
        diff_graph_vecs, graph_kl = self.rsample(
            diff_graph_vecs, self.G_mean, self.G_var)
        kl_div = tree_kl + graph_kl

        diff_tree_vecs = diff_tree_vecs.unsqueeze(
            1).expand(-1, x_tree_vecs.size(1), -1)
        diff_graph_vecs = diff_graph_vecs.unsqueeze(
            1).expand(-1, x_graph_vecs.size(1), -1)
...
class HierCondVGNN(HierVGNN):
    ...
    def __init__(self, args):
        ...
        self.W_tree = nn.Sequential(nn.Linear(
            args.hidden_size + args.latent_size + args.cond_size, args.hidden_size), nn.ReLU())
        self.W_graph = nn.Sequential(nn.Linear(
            args.hidden_size + args.latent_size + args.cond_size, args.hidden_size), nn.ReLU())
    ...
    def translate(self, tensors, cond, num_decode, enum_root):
        ...
        z_tree = torch.randn(
            num_decode, 1, self.latent_size).expand(-1, tree_vecs.size(1), -1).to(device)
        z_graph = torch.randn(
            num_decode, 1, self.latent_size).expand(-1, graph_vecs.size(1), -1).to(device)
        z_tree_vecs = self.W_tree(
            torch.cat([tree_vecs, z_tree, tree_cond], dim=-1))
