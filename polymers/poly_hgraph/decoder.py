...
class HierMPNDecoder(nn.Module):
    ...
    def init_decoder_state(self, tree_batch, tree_tensors, src_root_vecs):
        ...
        new_tree_tensors = tree_tensors[:2] + \
            [agraph, bgraph] + tree_tensors[4:]
        htree = HTuple()
        htree.mess = self.rnn_cell.get_init_state(
            tree_tensors[1], src_root_vecs)
    ...
    def get_topo_score(self, src_tree_vecs, batch_idx, topo_vecs):
        if self.use_attention:
            topo_cxt = self.attention(
                src_tree_vecs, batch_idx, topo_vecs, self.A_topo)
        else:
    ...
    def get_cls_score(self, src_tree_vecs, batch_idx, cls_vecs, cls_labs):
        if self.use_attention:
            cls_cxt = self.attention(
                src_tree_vecs, batch_idx, cls_vecs, self.A_cls)
        else:
    ...
    def get_assm_score(self, src_graph_vecs, batch_idx, assm_vecs):
        if self.use_attention:
            assm_cxt = self.attention(
                src_graph_vecs, batch_idx, assm_vecs, self.A_assm)
        else:
    ...
    def forward(self, src_mol_vecs, graphs, tensors, orders):
        ...
        init_vecs = src_root_vecs if self.latent_size == self.hidden_size else self.W_root(
            src_root_vecs)
        ...
        graph_tensors = self.hmpn.embed_graph(
            graph_tensors) + (graph_tensors[-1],)  # preprocess graph tensors
        ...
        for t in range(maxt):
            ...
            assert htree.emask[0].item() == 0 and hinter.emask[0].item(
            ) == 0 and hgraph.vmask[0].item() == 0 and hgraph.emask[0].item() == 0
            ...
            subtree = htree.emask.new_tensor(
                subtree[0]), htree.emask.new_tensor(subtree[1])
            htree.emask.scatter_(0, subtree[1], 1)
            hinter.emask.scatter_(0, subtree[1], 1)

            cur_tree_tensors = self.apply_tree_mask(
                tree_tensors, htree, hgraph)
            cur_inter_tensors = self.apply_tree_mask(
                inter_tensors, hinter, hgraph)
            ...
            for i in batch_list:
                ...
                inter_label = [(pos, self.vocab[(cls, icls)][1])
                               for pos, icls in inter_label]
                ...
                if len(tree_batch.nodes[xid]['cluster']) > 2:
                    ...
                    cand_vecs = self.enum_attach(
                        hgraph, cands, icls, nth_child)
        ...
        cls_loss = self.cls_loss(cls_scores, cls_labs) + \
            self.icls_loss(icls_scores, icls_labs)
        ...
        if len(all_assm_preds) > 0:
            ...
            assm_scores = self.get_assm_score(
                src_graph_vecs, batch_idx, assm_vecs)
    ...
    def decode(self, src_mol_vecs, greedy=True, max_decode_step=100, beam=5):
        ...
        root_cls, root_icls = root_cls.tolist(
        ), icls_scores.max(dim=-1)[1].tolist()
        ...
        for bid in range(batch_size):
            ...
            tree_batch.register_cgraph(
                root_idx, new_atoms, new_bonds, attached)
        ...
        # wiring root (only for tree, not inter)
        h[1: batch_size + 1] = init_vecs

        for t in range(max_decode_step):
            batch_list = [bid for bid in range(
                batch_size) if len(stack[bid]) > 0]
            if len(batch_list) == 0:
                break

            batch_idx = batch_idx.new_tensor(batch_list)
            cur_tree_nodes = [stack[bid][-1] for bid in batch_list]
            subtree = batch_idx.new_tensor(
                cur_tree_nodes), batch_idx.new_tensor([])
            subgraph = batch_idx.new_tensor(tree_batch.get_cluster_nodes(
                cur_tree_nodes)), batch_idx.new_tensor(tree_batch.get_cluster_edges(cur_tree_nodes))
            ...
            for i, bid in enumerate(batch_list):
                if topo_preds[i] > 0.5 and tree_batch.can_expand(stack[bid][-1]):
                    ...
                    edge_feature = batch_idx.new_tensor(
                        [stack[bid][-1], new_node, 0])  # parent to child is 0
            ...
            htree, hinter, hgraph = self.hmpn(
                tree_tensors, tree_tensors, graph_tensors, htree, hinter, hgraph, subtree, subgraph)
            cur_mess = self.rnn_cell.get_hidden_state(
                htree.mess).index_select(0, subtree[1])
            ...
            if len(expand_list) > 0:
                ...
                if not greedy:
                    # score is output of log_softmax
                    scores = torch.exp(scores)
                    shuf_idx = torch.multinomial(
                        scores, beam, replacement=False).tolist()
            ...
            for i, bid in enumerate(expand_list):
                ...
                for kk in cls_beam:  # try until one is chemically valid
                    ...
                    smiles, ismiles = self.vocab.get_smiles(
                        clab), self.vocab.get_ismiles(ilab)
                    ...
                    if len(inter_cands) == 0:
                        ...
                        icls = [self.vocab[(smiles, x)][1]
                                for x in anchor_smiles]
                        cands = inter_cands if len(attach_points) <= 2 else [
                            (x[0], x[-1]) for x in inter_cands]
                        cand_vecs = self.enum_attach(
                            hgraph, cands, icls, nth_child)

                        batch_idx = batch_idx.new_tensor(
                            [bid] * len(inter_cands))
                        assm_scores = self.get_assm_score(
                            src_graph_vecs, batch_idx, cand_vecs).tolist()
                        sorted_cands = sorted(
                            list(zip(inter_cands, assm_scores)), key=lambda x: x[1], reverse=True)
                ...
                if not success:  # force backtrack
                    # pop the dummy new_node which can't be added
                    child = stack[bid].pop()
                    nth_child = tree_batch.graph.in_degree(stack[bid][-1])
                    edge_feature = batch_idx.new_tensor(
                        [child, stack[bid][-1], nth_child])
